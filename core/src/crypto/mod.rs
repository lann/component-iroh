...
//! held as a `lann:webcrypto` handle (see `sign`).
//!
//! Everything else that was once here — the record-protection suite, key
//! exchange, verification — now comes from the `lann:tls` sibling's
//! curated crates (`lann-tls-quinn`), which run those surfaces in-guest
