...
// Guest knobs pass through the shim's GUEST_ENV.
globalThis.GUEST_ENV = {
  ...(process.env.BLOB_MB && { BLOB_MB: process.env.BLOB_MB }),
  ...(process.env.RELAY && { RELAY: process.env.RELAY }),
};
...
const { run } = await import("./generated/iroh-blobs.js");
