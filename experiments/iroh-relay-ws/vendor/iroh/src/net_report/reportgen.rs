...
async fn run_https_probe(
    ...
    // wasi (spike): there is no HTTP client path; the relay's true
    // reachability is proven by the websocket dial itself (a failed dial
    // surfaces on the relay connection). Report a nominal latency so
    // home-relay selection proceeds.
    #[cfg(wasm_wasi)]
    {
        return Ok(HttpsProbeReport {
            relay,
