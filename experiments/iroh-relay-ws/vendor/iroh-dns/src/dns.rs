...
impl HickoryResolver {
    ...
    fn system_config() -> Result<(ResolverConfig, ResolverOpts), hickory_resolver::net::NetError> {
        ...
        // wasi has no system resolver configuration (hickory cfg-gates
        // `read_system_conf` out); an empty config keeps construction
        // working — lookups fail, which relay-only deployments never do.
        #[cfg(target_os = "wasi")]
        let (system_config, options) = (
            hickory_resolver::config::ResolverConfig::default(),
            ResolverOpts::default(),
        );
        #[cfg(not(any(target_os = "android", target_os = "wasi")))]
