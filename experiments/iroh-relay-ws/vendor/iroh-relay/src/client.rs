...
impl ClientBuilder {
    ...
    #[cfg(all(not(wasm_browser), not(wasm_wasi)))]
    ...
    /// Establishes a new connection to the relay server.
    ///
    /// The wasi branch: the websocket lives host-side behind the datagram
    /// bridge (see [`datagram_pipe`]); subprotocol negotiation happens at
    /// the bridge and the in-band challenge handshake runs over the pipe,
    /// exactly like the browser branch.
    #[cfg(wasm_wasi)]
    pub async fn connect(&self) -> Result<Client, ConnectError> {
        use n0_error::StdResultExt;
        ...
        debug!(%dial_url, "Dialing relay over the datagram bridge");

        let (pipe, negotiated) =
            datagram_pipe::DatagramPipe::connect(&dial_url, ProtocolVersion::all())
                .await
                .anyerr()?;

        let protocol_version =
            ProtocolVersion::match_from_str(&negotiated).ok_or_else(|| {
                e!(ConnectError::BadVersionHeader {
                    server_version: Some(negotiated)
                })
            })?;

        let conn = Conn::new(pipe, self.key_cache.clone(), &self.secret_key, protocol_version)
            .await?;
